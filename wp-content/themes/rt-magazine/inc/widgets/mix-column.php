...
function RT_Magazine_Home_Featured_Action_Mix_Column() {

  register_widget( 'RT_Magazine_Home_Featured_Mix_Column' );
  
}
add_action( 'widgets_init', 'RT_Magazine_Home_Featured_Action_Mix_Column' );


/**
* 
*/
class RT_Magazine_Home_Featured_Mix_Column extends WP_Widget
{
	...
	function __construct()	{
		...
		$widget_ops = array(
		  'classname'   => 'mix-column default-padding',
		  'description' => esc_html__( 'Displays latest posts or posts from a choosen category.', 'rt-magazine' ),
		);		

		parent::__construct( 'RT_Magazine_Home_Featured_Mix_Column',esc_html__( 'RT Magazine: Mix Column', 'rt-magazine' ), $widget_ops, $control_ops );
	}
					...
					<div class="row">
	                	<?php while ( $the_query->have_posts() ) : $the_query->the_post(); $cn++; ?>
							...
							<?php 	
								...
								$image_size = 'rt-magazine-two-column';
								$image_class= '';														
								if( $cn == 1){									
									$image_size = 'rt-magazine-featured-column-list';
									$post_wrapper_class = 'custom-col-12';					
								}
